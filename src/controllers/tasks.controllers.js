...
const getUsuarioIndividual = async (req, res, next) => {
    try {
        const { id } = req.params;
        
        const result = await pool.query('select nombre, apellido, correoelectronico from usuario where dni  = $1', [id]);
        res.json(result.rows);
...
const getPacienteTurno = async (req, res, next) => {

    const { id } = req.params;

    try{
        const result = await pool.query('select * from turno_usuariopaciente where id_transaccion = $1', [id]);
        res.json(result.rows);
    } catch (error) {
        next(error);
    }
...
const agregarHistorialClinico = async (req, res, next) => {
    const { diagnostico,  estudios_solicitados, medicamentos, motivo} = req.body;
    const { id } = req.params;

    try{
        const result = await pool.query('insert into registrohistoriaclinica (id, motivo, diagnostico,  estudios_solicitados, medicamentos ) values($1, $2, $3, $4, $5) returning *',
        [id, motivo, diagnostico, estudios_solicitados, medicamentos])
...
const getHistoriaClinicaUsuario = async (req, res, next) => {
    try {
        const result = await pool.query('select * from historiaclinicaxpaciente');
        res.json(result.rows);
...
const getHistoriaClinicaUsuarioIndividual = async (req, res, next) => {

    const { id } = req.params;

    try {
        const result = await pool.query('select * from historiaclinicaxpaciente where dni = $1',[id]);
        res.json(result.rows);
...
const getTurnosMedico = async (req, res, next) => {
    
    try {
        const { id } = req.params;

        const result = await pool.query('select * from turno where id_usuariom = $1', [id]);
...
module.exports = {
    ...
    getHistoriaClinicaUsuario,
    getHistoriaClinicaUsuarioIndividual,
    getPacienteTurno,
